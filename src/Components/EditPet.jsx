...
const EditPetProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchPetDetails = async () => {
      try {
        const response = await api.get(`/pet/get/${petId}`);
        ...
        toast.error("Failed to fetch pet details. Please try again.");
      }
    };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name || !petname || !age || !breed || !size || !color || !gender || !location || !description || !medicalHistory || !status) {
    ...
    try {
      const response = await api.put(`/pet/update/${petId}`, formData, {
        headers: {
