...
const PetReviews = ({ petId }) => {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                const { data } = await api.get(`/review/pet/${petId}`, {
                    headers: {
    ...
    return (
        <div className="max-w-3xl mx-auto my-8 p-6 bg-white rounded-lg shadow-lg border border-gray-200">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-3xl font-semibold text-center text-indigo-700">Reviews for this Pet</h3>
                <button 
                    className="text-xl text-indigo-600 hover:text-indigo-800 transition duration-300"
            ...
            {isOpen && (
                <>
                    {reviews.length === 0 ? (
                        <p className="text-center text-lg text-gray-500">No reviews yet for this pet.</p>
                    ) : (
                        reviews.map((review) => (
                            <div key={review._id} className="review-card transition-all duration-300 hover:shadow-xl hover:scale-105 border-b-2 py-4 mb-6 bg-gray-50 rounded-lg">
                                <div className="flex ml-6 justify-between items-center">
                                    <p className="font-bold text-xl text-gray-800">{review.user.name}</p>
                                    <p className="text-yellow-500 mr-20  font-semibold mt text-2xl">{review.rating} Star{review.rating > 1 && 's'}</p>
                                </div>
