...
const AvailablePets = () => {
  ...
  const fetchPets = async () => {
    try {
      const response = await api.get("/pet/shelterPets?status=available");
  ...
  if (!pets || pets.length === 0) {
    return <div className="text-center text-xl font-bold text-gray-500 mt-12">No available pets</div>;
  }

  return (
    <div className="bg-gradient-to-r from-green-400 via-lime-500 to-teal-500 min-h-screen py-12 px-4">
      <div className="container mx-auto">
        <h2 className="text-center text-4xl font-bold text-white mb-8">Available Pets</h2>
